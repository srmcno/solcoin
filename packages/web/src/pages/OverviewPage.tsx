...
interface Overview {
  ...
  revenuePerLaunch?: { meanSol?: number; medianSol?: number; n?: number; reliable?: boolean };
  pipeline?: {
...
export function OverviewPage() {
  ...
  const launched = overview?.tokens?.launched ?? 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <OperatingStrip status={status} loading={statusQuery.isLoading} error={statusQuery.error} />
  ...
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
      <StatTile
        ...
        hint={
          fees?.outstandingAccruedSol !== undefined
            ? `${formatSol(fees.outstandingAccruedSol)} earned but not yet claimed`
            : 'Claimed on-chain'
        }
      />
      ...
      <StatTile
        ...
        hint={
          netProfit === null || netProfit === undefined
            ? `No SOL price has been recorded, so USD costs cannot be converted honestly. Excluding USD costs: ${formatSol(netProfitExclUsd)}.`
      ...
      <StatTile
        ...
        tone={wallet?.belowFloor ? 'negative' : 'neutral'}
        hint={
          wallet?.belowFloor
            ? `Below the ${formatSol(wallet.floorSol)} floor — spending is halted`
            : `Floor ${formatSol(wallet?.floorSol)}`
        }
      />
      <StatTile
        label="Tokens launched"
        value={formatNumber(overview?.tokens?.launched)}
        hint={`${formatNumber(overview?.tokens?.active)} active · ${formatNumber(overview?.tokens?.graduated)} graduated`}
      />
    </div>
  );
}
...
function GettingStartedTiles({ overview, status }: { overview: Overview | undefined; status: SystemStatus | undefined }) {
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <StatTile
        label="Wallet balance"
        value={formatSol(status?.wallet?.balanceSol)}
        tone={status?.wallet?.belowFloor ? 'negative' : 'neutral'}
        hint={status?.wallet?.address ? `Floor ${formatSol(status.wallet.floorSol)}` : 'No wallet configured yet'}
      />
      <StatTile
        label="Trends active"
    ...
    return (
      <Card>
        <EmptyState
          ...
          description="The status endpoint returned nothing. If the server has only just started, this resolves within a few seconds."
        />
      </Card>
    );
  }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
          <StripFact label="Network">
            <Badge tone={networkTone}>{network ? humanise(network) : 'Unknown'}</Badge>
          </StripFact>
...
function LimitMeter({
  ...
  const tone: Tone = !known ? 'neutral' : ratio >= 1 ? 'negative' : ratio >= 0.8 ? 'warning' : 'accent';
    ...
    <div>
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="tnum text-xs text-ink-subtle">
          {render(value)} / {limit === undefined ? '—' : render(limit)}
        </span>
      </div>
      ...
      <div className="mt-1 flex items-center justify-between gap-2 text-[11px]">
        <span className={state === 'at limit' ? 'text-negative' : state === 'near limit' ? 'text-warning' : 'text-ink-subtle'}>
          {known ? `${formatPercent(Math.min(ratio, 1), 0)} used${state ? ` — ${state}` : ''}` : 'No limit reported'}
        </span>
      </div>
    </div>
  );
...
function FeeChart({ query }: { query: { isLoading: boolean; isError: boolean; error: Error | null; data: SeriesResponse | undefined; refetch: () => unknown } }) {
  ...
  if (points.length === 0 || !hasValue) {
    return (
      <EmptyState
        title="No creator fees collected in the last 30 days"
        description="Fees appear here once a launched token has earned and a collection transaction has landed. Until then this chart has nothing honest to draw."
        action={
...
function buildAttention(overview: Overview | undefined, status: SystemStatus | undefined): AttentionItem[] {
  ...
  const wallet = status?.wallet;
  if (wallet?.belowFloor) {
    items.push({
      id: 'wallet-floor',
      tone: 'negative',
      title: 'Wallet is below its floor',
      detail: `The operating wallet holds ${formatSol(wallet.balanceSol)} against a floor of ${formatSol(wallet.floorSol)}. Spending is halted until it is topped up.`,
      to: '/wallet',
      cta: 'Open wallet',
    });
  } else if (wallet && wallet.canSign === false && status?.network !== 'simulation') {
    items.push({
      id: 'wallet-sign',
      tone: 'warning',
      ...
      {items.length === 0 ? (
        <div className="mt-2">
          <EmptyState
            ...
            description="No approvals are queued, the wallet is above its floor, every job ran cleanly and no provider is reporting trouble."
          />
        </div>
      ) : (
...
function RevenuePerLaunch({ overview }: { overview: Overview | undefined }) {
  ...
  const ratio = median !== undefined && median > 0 && mean !== undefined ? mean / median : null;
  const rate = overview?.successfulLaunchRate;
      ...
      {n === 0 ? (
        <div className="mt-2">
          <EmptyState
            ...
            description="This fills in once tokens have been launched and their fees observed. Until then there is no distribution to describe."
          />
        </div>
      ) : (
        <div className="mt-4 space-y-3">
          ...
          {rate && rate.n !== undefined && (
            <div className="border-t border-border pt-3">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <span className="text-xs font-medium text-ink-muted">
                  Successful launches (10+ organic holders)
                </span>
              ...
              {rate.reliable === false && (
                <div className="mt-2">
                  <Note tone="warning">
                    This rate comes from {formatNumber(rate.n)} launches. The interval above is nearly as wide as the
                    range it sits in, so the point estimate is not yet informative.
                  </Note>
                </div>
              )}
            </div>
