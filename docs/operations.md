...
| `launch-recovery` | 180 s | 120 s | no | — | Resolves launches broadcast but never confirmed. Waits 2 minutes before intervening; treats the mint account existing as definitive; gives up as `expired` after 10 minutes; finishes the post-confirmation work for a launch that confirmed while nobody was watching. Also restores candidates left in `launching` with no launch row at all after 10 minutes — the state a process death between marking the concept and claiming the launch leaves behind, which nothing else looks at. A candidate whose launch *was* claimed is never restored: that attempt decides its own outcome. |
...
| Launching stops: `N consecutive launch failures on <network>. Launching is halted until the cause is resolved and the failures are acknowledged (POST /api/system/clear-launch-failures).` | `limits.consecutiveFailureShutdown` (default **3**) reached. Separately, the launch failure handler calls `guard.autoStop()` at the same threshold, which engages the global emergency stop. | Find the *first* failure — the later ones are usually the same cause. Fix it, then **two** calls, both needing `emergency_stop` and a reason: `POST /api/system/clear-launch-failures` to acknowledge the failures, and `POST /api/system/emergency-release`. Acknowledging reclassifies the failed rows as `abandoned`, which keeps them in the history and the failure analytics but takes them out of the breaker's window — and retires their idempotency keys, so those concepts can be attempted again. The counter cannot clear itself by a launch succeeding: while the breaker is engaged no launch runs, so there is no success to be had. |
